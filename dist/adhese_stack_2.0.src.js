...
Adhese.prototype.checkPreviewList = function() {
    ...
    previewAds.forEach(ad => {
        this.previewFormats[ad.format + (ad.position ? ad.position : "")] = {
            slot: ad.slotId ? ad.slotId : "",
            creative: ad.cId,
            templateFile: ad.format,
            width: 0,
            height: 0,
            position: ad.position ? ad.position : ""
        };
    });
...
Adhese.prototype.renderAd = function(ad) {
    ...
    if (this.config.safeframe === true) this.safeframe.render(ad.containingElementId); else this.friendlyIframeRender(ad.ToRenderAd, ad.containingElementId);
...
Adhese.prototype.getAds = async function(ads, previewURL) {
    try {
        ...
        if (this.config.requestType === "GET" || typeof previewURL !== "undefined") {
            url = typeof previewURL !== "undefined" && previewURL ? previewURL : this.getMultipleRequestUri(Object.values(ads), {
                type: "json"
            });
        } else {
...
Adhese.prototype.lazyRequestAds = function(changes, observer) {
    changes.forEach(async element => {
        if (!element.target.dataset.loaded && element.intersectionRatio === 1) {
            ...
            if (this.config.safeframe === true) this.safeframe.addPositions([ this.ads.lazyRequest[element.target.id].ToRenderAd ]);
